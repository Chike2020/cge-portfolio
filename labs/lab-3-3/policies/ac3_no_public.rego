...
deny contains msg if {
    ...
    msg := sprintf(
        "[AC-3] %s: bucket allows public access. Set uniform_bucket_level_access=true and public_access_prevention=\"enforced\".",
        [resource.address],
    )
}
...
# --- Check Firewalls for Open Management Ports ---
...
deny contains msg if {
    ...
    mgmt_port(port)
    msg := sprintf(
        "[AC-3] %s: management port %s open to %s. Narrow source_ranges or remove the rule.",
        [r.address, port, src],
    )
}